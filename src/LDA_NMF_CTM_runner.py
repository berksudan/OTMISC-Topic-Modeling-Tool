...
def runner(args):
    ...
    # Make dataset for LDA/NMF
    # These two algorithms don't need val and test set
    if args['model_name'] in ['lda', 'nmf']:
        ...
        for i in range(len(args['docs'])):
            if len(args['docs'][i].split()) >= 3:     # Delete data that cannot be recognized by OCTIS
                result = []
                result.append(documents[i])
                if i < len(args['docs']) * PT_TRAIN:
                    result.append('train')
                elif i >= len(args['docs']) * PT_TRAIN and i < len(args['docs']) * (PT_TRAIN + PT_VAL):
                    result.append('val')
                else:
                    result.append('test')
                result.append(args['labels'][i])
    ...
    if args['model_name'] == 'lda':
        ...
        rand = str(round(np.random.rand(), 6))
        OCTIS_model = CTM(bert_path="/content/drive/MyDrive/SS_2022_Praktikum/for CTM/try" +\
                          rand, num_topics = args['num_topics'], num_epochs=30)                              # Create Model
        output = OCTIS_model.train_model(dataset)                                                            # Train the model
